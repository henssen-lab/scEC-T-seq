...
root_temp=chp212_plate5a6
data=chp212_plate5a6/chp212_plate5a6_metadata_new.txt

# 1. Merge fastq (generate pseudobulk)
set -e
set -x

tail -n+2 $data | awk '{print $18"_R1_001_val_1.fq.gz"}' | xargs gunzip -c > ${root_temp}/R1.fq
gzip ${root_temp}/R1.fq

tail -n+2 $data | awk '{print $18"_R2_001_val_2.fq.gz"}' | xargs gunzip -c > ${root_temp}/R2.fq
gzip ${root_temp}/R2.fq

# 2. Run Mapping
cd ${root_temp}
STAR --runThreadN 24 --genomeDir $dir  --genomeLoad NoSharedMemory \
--readFilesIn ${root_temp}/R1.fq.gz ${root_temp}/R2.fq.gz --readFilesCommand zcat \
...
# 3. Downsampling
threads=16
samtools view -@ $threads -s 0.30 -O BAM -o Aligned.out.downsample30.bam -T $ref Aligned.out.bam
samtools sort -@ $threads -o Aligned.out.downsample30.sorted.bam Aligned.out.downsample30.bam
samtools index Aligned.out.downsample30.sorted.bam


# 4. Fusion genes calling
arriba -x Aligned.out.downsample30.sorted.bam -o fusions.tsv -O fusions.discarded.tsv \
-a $ref -g $anno \
-F 150 -U 700 \
...
# 5. Filter
tail -n+2 fusions.tsv | awk '$13 >= 50 && $14 >= 50 && 2*($10+$12+$11)/($13+$14) > 0.3 {print $0}' > fusions_confident.tsv
cat fusions_confident.tsv | awk '{split($5, a, ":"); split($6, b, ":"); print a[1]"\t"a[2]-1"\t"a[2]"\t"b[1]"\t"b[2]-1"\t"b[2]}' > fusions_confident.bedpe
